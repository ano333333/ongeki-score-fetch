...
import userScoreDataType from "./utils/userScoreDataType";
import runtimeMessageType from "./utils/runtimeMessageType";
import scoreConstDataType from "./utils/scoreConstDataType";
...
chrome.runtime.onMessage.addListener(function (
    ...
    if (message.type === "user_score_domparse_start") {
        const { html, diff } = message;
        parseUserScoreHTML(diff, html)
            .then((datas) => {
                sendResponse({
                    type: "domparse_end",
                    diff,
    ...
    } else if (message.type === "score_const_domparse_start") {
        const { html } = message;
        parseScoreConstHTML(html).then((datas) => {
            sendResponse({
...
async function parseUserScoreHTML(dif: string, html: string) {
...
async function parseScoreConstHTML(html: string) {
    const domparser = new DOMParser();
    const doc = domparser.parseFromString(html, "text/html");
    if (doc.getElementsByTagName("parseerror").length) {
        throw new Error("HTML Parse Error");
    ...
    const datas: scoreConstDataType[] = [];
    for (const tr of trs) {
        if (tr instanceof HTMLTableRowElement) {
            ...
            const constNum = parseFloat(constStr);
            datas.push({
                name,
                difficulty,
                const: constNum,
